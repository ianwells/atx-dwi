...
So we can get a sense that, for the years 2008-2011, monthly DWI's seem to be decreasing (good job APD), and any monthly trend doesn't really jump out at us (there might be one in December). 
...
ggplot(data=apd.monthly, aes(x = date, y = dwi, group = year, color = factor(year))) + geom_point()

ggplot(data=apd.monthly, aes(x = (month(date)), y = dwi, group = year, color = factor(year))) + geom_line() + scale_x_continuous( breaks = 1:12, labels = label.months)
