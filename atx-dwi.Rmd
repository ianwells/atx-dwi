...
ggplot(data=dwi.monthly, aes(x = (month(date)), y = count, group = year, color = factor(year))) + geom_smooth(se = FALSE) + scale_x_continuous( breaks = 1:12, labels = label.months)

```

So we can get a sense that, for the years 2008-2011, monthly DWI's seem to be decreasing (good job APD), and as for a monthly trend there might be seasonal peaks in January, December, and maybe August? 

Now, I'm curious to see how the Chief's Monthly Report compares.  Let's mark when Uber was operating.
...
ggplot(data=apd.monthly, aes(x = date, y = dwi, color = factor(year))) + uber + geom_point() 

ggplot(data=apd.monthly, aes(x = (month(date)), y = dwi, group = year, color = factor(year))) + geom_smooth(se = FALSE) + scale_x_continuous( breaks = 1:12, labels = label.months)
...
```{r warning = FALSE}
dwi.monthly$date <- as.Date(dwi.monthly$date)
tabc.monthly$date <- as.Date(tabc.monthly$date)

dpr.monthly <- merge(dwi.monthly,tabc.monthly, by = 'date')
dpr.monthly$dwi_per_mm <- dpr.monthly$count/dpr.monthly$rev

ggplot(data=dpr.monthly, aes(x = date, y = dwi_per_mm, group = year.x, color = factor(year.x))) + geom_point()

ggplot(data=dpr.monthly, aes(x = rev, y = count, color = year.x)) + geom_point() + geom_smooth(method = "lm")

summary(lm(data = dpr.monthly, count ~ rev))
